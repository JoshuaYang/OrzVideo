(function(window) {
	...
	jsmpeg.prototype.findStartCode = function(code) {
		var current = 0;
		while (true) {
			if (typeof this.buffer.findNextMPEGStartCode != "function") {
				break;
			}
	...
	jsmpeg.prototype.decodePicture = function(skipOutput) {
		...
		// this.seeking = false;
		if (!this._muted && this.audio && !this.audio.paused && Math.abs(this.audio.currentTime - this.currentTime) >= 0.3) {
			if (this.audio.currentTime == this._audioCurrentTime) return;
			this._audioCurrentTime = this.audio.currentTime;
			var currentFrame = Math.round(this.audio.currentTime * this.pictureRate);
			if (currentFrame > this.currentFrame || Math.abs(currentFrame - this.currentFrame) > 0.3) {
				this.currentFrame = currentFrame;
			} else {
				return;
			}
			var target = null;
			for (var i = 0; i < this.intraFrames.length && this.intraFrames[i].frame <= this.currentFrame; i++) {
				target = this.intraFrames[i];
			}
			this.buffer.index = target.index;
		} else {
